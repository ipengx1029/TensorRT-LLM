...
QwenModelInfer<sms_count, max_share_memory>::QwenModelInfer() {
    mAttnScale = 1.0f / sqrt(QWEN3_1P7B_HEAD_DIM);
    mRmsNormEps = 1e-06;
}
template<int sms_count, int max_share_memory>
void QwenModelInfer<sms_count, max_share_memory>::infer(
...
TGlobal QwenModelInfer<sms_count, max_share_memory>::make_global_configs(MKGlobals* glob) {
    auto &globals = *glob;
    ...
    return TGlobal(
        ...
        // kv cache
        convert2gl_array<kvcache_layout, QWEN3_1P7B_NUM_LAYERS>(globals.kv_caches),
        // other buffers
